...
The `sql/canonical_queries.sql` file needs to be opened and verified. It should be populated with a set of SQL queries suitable for performance testing. For the initial run, using a simple query like `SELECT 1;` is sufficient to test the mechanism, but more representative queries can be used.
...
##### 4.3.2.7. `src/profiling_modules/metrics_performance.py`
  ...
  * **Purpose**: A dedicated module to run performance benchmarks. It reads a set of predefined SQL queries and times their execution against the database.
...
All preceding tasks (1.2, 1.3, 2.1) must be successfully completed. All six databases (4 legacy, 2 benchmark) must exist on the PostgreSQL server. The `src/profiling_modules/` package must be fully populated with the seven Python files from Task 2.1. Confirm `sql_queries_dir` key in `[paths]` section of `src/config.ini` points to `sql/` directory. Confirm `benchmark_dbs` key lists the two correct benchmark database names. A new file, `sql/canonical_queries.sql`, must exist and be populated with a set of representative SQL queries for performance benchmarking. For the initial run, `SELECT 1;` and `SELECT pg_sleep(0.1);` are sufficient.
...
3.  **New File Creation (`sql/canonical_queries.sql`)**:
      * A new file, `sql/canonical_queries.sql`, must exist and be populated with a set of representative SQL queries for performance benchmarking. For the initial run, `SELECT 1;` and `SELECT pg_sleep(0.1);` are sufficient.
      * **Currrent Sample Content for `sql/canonical_queries.sql`**: Below is the current starting set of queries currently saved in `sql/canonical_queries.sql`. These are designed to test common analytical operations like joins, aggregations, and filtering.
        ```sql
        ...
        ```
      * **Refinement**: For the initial run, using `SELECT 1;` is sufficient to test that the performance benchmarking *mechanism* works. The queries can be made more complex later.
...
1.  **Log File Review**:
      ...
      * Scan the log for any lines containing `ERROR` or `CRITICAL`. This is the fastest way to diagnose problems. A clean run will have only `INFO` and `WARNING` messages.
...
3.  **Content Spot-Check**:
      * **Action**: Open `outputs/metrics/tmp_df9_table_metrics.csv`.
      * **Verify**: The file should contain rows for the `TMP_DF9` tables (e.g., `location`, `description`, `archInterp`). The columns should match the metrics (e.g., `row_estimate`, `total_size`, `bloat_percent`).
      * **Action**: Open `outputs/metrics/tmp_benchmark_wide_numeric_column_profiles.csv`.
      * **Verify**: The file should contain rows for all columns created by `flatten_df9.sql`. Check that the `tablename` column consistently shows `wide_format_data`.
      * **Action**: Open `outputs/metrics/tmp_df8_interop_metrics.json`.
      * **Verify**: The file should be a simple JSON object containing keys like `"jdi"`, `"lif"`, and `"nf"` with numerical values.
