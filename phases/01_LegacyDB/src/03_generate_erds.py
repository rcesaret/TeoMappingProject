...
def generate_and_save_erd(
    ...
    try:
        ...
        logging.error(
            f"Failed to generate ERD for '{graph_title}'. Error: {e}", exc_info=True
        )
...
def main() -> None:
    ...
    for i, db_name in enumerate(all_dbs_to_profile, 1):
        ...
        if not engine:
            logging.error(
                f"Skipping ERD generation for '{db_name}' due to connection failure."
            )
            continue
        ...
        try:
            ...
            logging.error(
                f"Could not reflect schema '{schema_name}'. Skipping. Error: {e}"
            )
            continue
