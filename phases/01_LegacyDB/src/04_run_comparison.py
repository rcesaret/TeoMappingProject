...
def load_all_metrics(input_dir: Path) -> Dict[str, Dict[str, Any]]:
    ...
    # Known metric file suffixes generated by the profiling pipeline
    metric_suffixes = [
        "basic_metrics",
        "schema_counts",
        "table_metrics",
    ...
    for file_path in metric_files:
        try:
            ...
            metric_name = None
            db_name = None
            # Strip a known suffix from the right; the remaining prefix is
            # treated as the database name.

            for suffix in metric_suffixes:
                full_suffix = f"_{suffix}"
                if stem.endswith(full_suffix):
                    metric_name = suffix
                    db_name = stem[: -len(full_suffix)]
                    break
...
def generate_markdown_report(
    summary_df: pd.DataFrame, all_data: Dict[str, Dict[str, Any]], output_path: Path
) -> None:
    ...
    report_parts.append("\n## 1. Executive Summary")
    report_parts.append(
        "This table provides a high-level overview of the key profiling metrics across all analyzed databases."
    )
    ...
    if perf_data:
        ...
        pivot_perf = perf_df.pivot(
            index="query_name", columns="database", values="latency_ms"
        )
        report_parts.append(
            "Latency (in milliseconds) for canonical queries. Lower is better."
        )
    ...
    report_parts.append(
        f"- **Total Raw Metric Files Found**: {sum(len(v) for v in all_data.values())}"
    )
